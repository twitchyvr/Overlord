...
export class TeamPanel extends PanelComponent {
    ...
    // IMPORTANT: agent.status from the server is the *registration* status (e.g. 'active'
    // for ALL agents in the DB, or 'WORKING' for ALL agents from agents/index.js) and is
    // NOT a reliable processing indicator. Only ses.isProcessing from agent_session_state
    // events reflects actual real-time processing.
    ...
    _buildAgentCard(agent) {
        ...
        // Derive visual status from processing state, not agent.status (which is always 'active'/'WORKING')
